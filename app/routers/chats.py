...
def mark_chat_read(
    db: Session, chat_id: uuid.UUID, user_id: uuid.UUID, message_id: int
) -> int | None:
    stmt = (
        update(ChatUser)
        .where(
...
def update_read_mesage(
    ...
    if not last_read:
        raise HTTPException(
            status_code=404, detail="Chat not found or you are not a member"
        )
...
def add_user_to_chat(
    ...
    new_row = ChatUser(chat_id=chat.id, user_id=add_user.id)
...
def create_chat(
    ...
    if payload.type == "direct":
        ...
        new_chat = Chat(type="group", name=payload.name)
        db.add(new_chat)
        db.flush()
...
def get_all_chats(
    ...
    for chat in direct_chats:
        ...
        previews.append(
            ChatPreview(
                ...
                avatar_url=other_user.avatar_url if other_user.avatar_url else None,
            )
        )
    ...
    previews.sort(
        key=lambda p: p.last_message_time or datetime.min.replace(tzinfo=timezone.utc),
        reverse=True,
    )
