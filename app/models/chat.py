...
class Chat(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("""
            (type = 'direct' and user_a_id is not null and user_b_id is not null and user_a_id < user_b_id)
            or
            (type = 'group' and user_a_id is null and user_b_id is null)""", name="check_valid_table_variables"),
        Index(
            "idx_unique_direct_chat",
            "user_a_id", "user_b_id",
            unique=True,
            postgresql_where=(type == "direct"),
        ),
    )